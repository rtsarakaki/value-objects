...
import { MustEndWithAlphaNumeric } from './MustEndWithAlphaNumeric.validation';
...
describe('Invalid values', () => {
	...
	describe.each(arrayOfValidValues)(`%p`, ({ value, label }) => {
		...
		const result = MustEndWithAlphaNumeric(value as string, label)
		...
		test(`Error message is ${result?.message}.`, () => {
			expect(result?.message).toEqual(`${label} must end with a letter or number.`)
		})
...
describe('Invalid label', () => {
	...
	test.each(arrayOfInvalidLabels)(`%p is valid.`, (label) => {
		const result = MustEndWithAlphaNumeric('ok', label as string)
		expect(result).toBeInstanceOf(InvalidValue)
